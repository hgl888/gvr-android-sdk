...
public class GLPanoView {
    ...
    private static final String[] SCENE_OBJS = { "sphere.obj"};
...
//        }).start();
    ...
    private void setObjFile(String filename, int sceneType){
        GLMesh mesh = GLObjFileUtils.LoadMesh(mContext.getResources(), filename);
...
//        state.pushMatrix();
    ...
    public void createTexture(){
        ...
        bitmap = null;
        if (mLeftResId > -1){
            InputStream is = mContext.getResources().openRawResource(mLeftResId);
            try {
                bitmap = BitmapFactory.decodeStream(is);
            } finally {
                try {
